...
# ---------------------------------------------------------------------
# Punt fix per a omega i mida final teòrica
# ---------------------------------------------------------------------
def punt_fix_omega(
    T: float,
    pk: np.ndarray,
...
def mida_final_teorica(tau: float, gamma: float, pk: np.ndarray, k_mitja: float) -> float:
    """
...
# ---------------------------------------------------------------------
# EPN: mida final simulada via HOUT
# ---------------------------------------------------------------------
def fraccio_HOUT(H: nx.DiGraph) -> float:
    """
    Aproxima la mida final via EPN:
      A_sim ≈ |HOUT|/N
    on HOUT són els nodes assolibles des de la HSCC en el DAG de condensació.
    """
    N = H.number_of_nodes()
    ...
    # HOUT: HSCC + tots els descendents al DAG de condensació
...
# ---------------------------------------------------------------------
# Funció GENERAL: entra un graf NX ja generat i retorna Ath i Asim
# ---------------------------------------------------------------------
def mida_final_epidemia_en_graf(
    ...
    (Ath, Asim):
      ...
      Asim[i] = estimació EPN: mitjana de |HOUT|/N sobre percolacions_per_tau
    """
    # --- Teoria (el graf és fix => pk fix)
    ...
    # --- Simulació EPN: per cada tau, fem diverses percolacions
    ...
    for i, tau in enumerate(tau_vals):
        ...
        for _ in range(percolacions_per_tau):
            _ = rng.integers(0, 2**31 - 1)  # consumim RNG per variabilitat reproductible
...
# ---------------------------------------------------------------------
# PLOTS: error i (opcional) corbes Ath vs Asim
# ---------------------------------------------------------------------
def plot_error_mida_final(
    ...
    fitxer_sortida: str | None = None,
    mostrar: bool = True,
):
    """
    Plot de l'error Asim - Ath (com el que tenies).
    """
...
def plot_mida_final_teoria_vs_sim(
    ...
    """
    Plot Ath vs Asim (per si també el vols, anàleg al de probabilitat d'epidèmia).
    """
    fig, ax = plt.subplots(figsize=(7.2, 4.6))
